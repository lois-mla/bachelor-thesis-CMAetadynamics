...
class MolSimCMA:
    def __init__(self, width, height, initial_size, increment_step, final_size, nsteps, cma_number_of_generations, cma_sigma, cma_upper_bound, cma_lower_bound=0):
        self.width = width
        self.height = height
        ...
        self.number_of_gaussians = initial_size
        self.restart = 0
    ...
    def update_hills(self, x, gen, sample):
        ...
        with open(f"{self.output_path}/HILLS/restart-{self.restart}-gen{gen}-sample{sample}-HILLS", "w") as hills:
            hills.write(content)
    ...
    def run_simulation(self, x, gen, sample):
        ...
        #
        metad: METAD ARG=phi,psi PACE=500000000 HEIGHT=1.2 SIGMA=0.35,0.35 FILE={self.output_path}/HILLS/restart-{self.restart}-gen{gen}-sample{sample}-HILLS

        # monitor the two variables and the metadynamics bias potential
        PRINT STRIDE=10 ARG=phi,psi,metad.bias FILE={self.output_path}/COLVAR/restart-{self.restart}-gen{gen}-sample{sample}-COLVAR
        """
        cvs = ["phi", "psi"]
        ...
        colvar_data = np.loadtxt(f'{self.output_path}/COLVAR/restart-{self.restart}-gen{gen}-sample{sample}-COLVAR')
        ...
        hist = np.histogram2d(phi, psi, bins=bins, range=range, density=None)
        # print("hist", hist)
    ...
    def find_mean(self, n):
        ...
        bias_script = f"""
            ...
            psi: TORSION ATOMS=7,9,15,17
            #
            metad: METAD ARG=phi,psi PACE=500 HEIGHT=0 SIGMA=0,0 FILE={self.output_path}/HILLS/HILLS_no_bias_{n}

            # monitor the two variables and the metadynamics bias potential
            PRINT STRIDE=10 ARG=phi,psi,metad.bias FILE={self.output_path}/COLVAR/COLVAR_no_bias{n}
            """
        ...
        colvar_data = np.loadtxt(f"{self.output_path}/COLVAR/COLVAR_no_bias{n}")
    ...
    #     mean = np.array(self.find_mean(restart) * self.number_of_gaussians)

    #     # initialize optimizer
    #     optimizer = SepCMA(mean=mean, sigma=self.cma_sigma, bounds=bounds_array, seed=0, population_size=self.starting_population_size)
    ...
    #                 popsize = optimizer.population_size * self.pop_increase_factor
    #                 mean = np.array(self.find_mean(restart) * self.number_of_gaussians)
    #                 optimizer = SepCMA(mean=mean, sigma=self.cma_sigma, population_size=popsize)
    ...
    def CMA(self):
        ...
        bounds_array = np.array([[self.cma_lower_bound, self.cma_upper_bound]] * (self.initial_size*2))

        mean = np.array(self.find_mean(self.restart) * self.initial_size)

        # initialize optimizer
        optimizer = SepCMA(mean=mean, sigma=self.cma_sigma, bounds=bounds_array)

        self.number_of_gaussians = self.initial_size
        # current_mean = initial_vector

        while self.number_of_gaussians <= self.final_size:
            for generation in range(12):  # Or some other stopping criterion
                solutions = []
                for sample in range(optimizer.population_size):
                    # ask optimizer for a sample
                    x = optimizer.ask()
            ...
            new_size = min(self.number_of_gaussians + self.increment_step, self.final_size)
            
            # Get the current best solution
            current_best = optimizer._mean

            self.restart += 1

            mean = np.array(self.find_mean(self.restart) * (new_size - self.number_of_gaussians))
            print("mean", mean, mean.shape)

            # Extend the current best solution to the new size
            extended_best = np.hstack((current_best, mean))
            print("extended_best", extended_best, extended_best.shape)

            bounds_array = np.array([[self.cma_lower_bound, self.cma_upper_bound]] * (new_size*2))


            
            # Reinitialize CMA-ES with the extended vector size
            optimizer = CMA(mean=extended_best, sigma=self.cma_sigma, bounds=bounds_array)
            
            # Update the current vector size
            self.number_of_gaussians = new_size

        # Final optimization with the full vector size
        # for generation in range(100):  # Or some other stopping criterion
        #     solutions = []
        ...
        # Get the final optimized solution
        final_solution = optimizer._mean
...
def plot_cvs_and_heights(cmaObj, gen, cvs, population_size, dir, solutions=None):
    ...
    for sample in range(population_size):
        ...
        colvar_data = np.loadtxt(f"{cmaObj.output_path}/COLVAR/restart-{cmaObj.restart}-gen{gen}-sample{sample}-COLVAR")
        hills_data = np.loadtxt(f"{cmaObj.output_path}/HILLS/restart-{cmaObj.restart}-gen{gen}-sample{sample}-HILLS")
    ...
    plt.suptitle(f"restart={cmaObj.restart}, noG={cmaObj.number_of_gaussians}")

    plt.savefig(f'{dir}/restart-{cmaObj.restart}-generation{gen}.png', bbox_inches='tight')
...
def plot_cvs_per_generation(gen, cvs, population_size, solutions=None):
    ...
    for sample in range(population_size):
        ...
        colvar_data = np.loadtxt(f"restart-{self.restart}-gen{gen}-sample{sample}-COLVAR")
...
if __name__ == "__main__":
    ...
    cma_sigma = 0.2 * (cma_upper_bound - cma_lower_bound)


    test = MolSimCMA(width, height, initial_size, increment_step, final_size, time_steps, cma_number_of_generations, cma_sigma, cma_upper_bound, cma_lower_bound)
    test.CMA()
